...
typedef enum {
    TUNNEL_LOCAL  = 0,  // Локальный проброс (клиент -> сервер -> цель)
    TUNNEL_REMOTE = 1,  // Удалённый проброс (сервер -> клиент -> цель)
    TUNNEL_DYNAMIC = 2, // SOCKS5 прокси
} TunnelType;
typedef struct {
    TunnelType type;
    uint16_t listen_port;       // Порт для прослушивания
    char remote_host[256];      // Куда пробрасывать
    uint16_t remote_port;       // Порт назначения
    int enabled;
} TunnelConfig;
typedef struct {
    TunnelConfig config;
